...
계획잡고 리팩토링한다고 하면 뭔가 잘못만든다는 것을 다시 고친다고 생각할 것이다. 막상 계획대로 하려고 해도 급한일이 생기면 그것먼저 처리한다고 제대로 하지도 못할거다. 리팩토링에 대해 이해하지 못하는 사람을 상대한다면 정말 저자의 말처럼 할 수 밖에 없을 것이다. 내가 맡은 일을 얼마나 다했느냐 묻는다면 100프로 했다하지말고 70프로 정도 했다고 말해라. 30프로 정도 버퍼를 만들어서 잘 활용하는 조언을 받았었다. 문제는 요즘은 git에 개발 히스토리가 다 남는다. 개발에 대해서 아는 매니저는 더 쪼아대는 경향도 있을 것이다.

온라인 상에 올라온 코드만보고 하는 것보다 옆에 앉아서 하는 리뷰를 더 선호한다. 리뷰를 통해 뭔가를 제안한다는 것은 리팩터링하라는 이야기이다. 물론 rdb -> nosql과 같이 커다란 리팩토링은 날을 잡아서 한다.

미래의 우리가 만든 프로그램이 어떤 식으로 바뀔지 모르니까 일단 작동하게 만드는 것. 이것이 기술부채이다. 그리고 기술부채를 갚는 것이 리팩토링이다.

6개월 뒤에 코드를 봤을 때 이해하지 못할수도 있다. 그렇다면 리팩토링하는 것이 중요할 것이다. 이해를 위한 리팩토링은 중요하다. 그러고나먄 중요한 리팩토링을 할 수 있을 것이다.
